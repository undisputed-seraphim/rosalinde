...
#pragma pack(pop)
	...
	for (bool done = false; !done;) {
		...
		for (int t = 0; t < IDLE.second.size(); ++t) {
			auto& step = trackstep[t];
			auto& time = tracktime[t];
			const auto& track = IDLE.second[t];
			const auto& tl = track.keyframes[time];
			const auto& kf1 = scarlet_quad.keyframes()[tl.keyframe_id];
			if (kf1.layers.empty()) {
				continue;
			}

			const auto& tl2 = (time == track.keyframes.size() - 1) ? track.keyframes[0] : track.keyframes[time + 1];
			const auto& kf2 = scarlet_quad.keyframes()[tl2.keyframe_id];

			vertices.storage().clear();
			indices.storage().clear();

			const float interpolation = (float(step) / float(tl.frames));
			...
			for (int k = 0; k < kf1.layers.size(); ++k) {
				...
				for (int j = 0; j < 4; ++j) {
					const auto& _1 = layer1.dst[j];
					const auto& _2 = layer2.dst[j];
					auto interpolated_pos = interpolated(_1, _2, interpolation);
					vertices.storage().emplace_back(vertex{
						layer1.texid,
						layer1.src[j] * texdim,
						//glm::vec3{layer1.dst[j], depth},
						glm::vec3(interpolated_pos, depth),
						layer1.fog[j]
					});
				}
