...
void v77::print_to_file() const {
	...
	for (const auto& s : s9) {
		ofs << std::format(
			fmts9, s.name, s.sa_set_id, s.sa_set_no, s.sa_set_main, s.sa_sb_set_id, s.sa_sb_set_no, s._0_1);
	}
	...
	for (const auto& s : sa) {
		ofs << std::format(
			fmtsa, s.s8_id, s.s8_no, s.s8_sum, s.s8_sum_once, s._unk0, s.sb_id, s.sb_no, s.s8_st, s._unk1, s._pad);
	}
